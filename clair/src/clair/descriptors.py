...
class TypeTag(object):
    ...
    def is_instance_T(self, object_):
...
class ListT(TypeTag):
    ...
    def is_instance_T(self, object_):
        if not isinstance(object_, list):
            return False
        for v in object_:
            if not self.valueT.is_instance_T(v):
                return False
        return True
...
class DictT(TypeTag):
    ...
    def is_instance_T(self, object_):
        ...
        for k, v in object_.iteritems():
            if not (self.keyT.is_instance_T(k) and self.valueT.is_instance_T(v)):
                return False
        return True

def is_instance_T(object_, tag):
    """Equivalent to `isinstance` but with TypeTag"""
    assert isinstance(tag, TypeTag), "`tag` must be a `TypeTag`."
    return tag.is_instance_T(object_)
...
class FieldDescriptor(object):
    ...
    def __init__(self, name, data_type, default_val, comment):
        ...
        assert is_instance_T(default_val, SumT(data_type, NoneT)), \
